...
class RocketViewModel extends ChangeNotifier {
  ...
  void setupRocket(Flight? currentFlight) {
    ...
    rocketCollection = FirebaseFirestore.instance
        .collection('flights')
        .doc(flight?.uniqueId)
        .collection('rocket');
    flightLoop(const Duration(
        milliseconds:
            500)); //Check status every 500ms, might want to go faster in a real flight.
  }
  ...
  Stream<double> getAltitudeStream() {
    return Stream<double>.periodic(
        const Duration(seconds: 1), (x) => rocket.altitude!);
  }
  ...
  void calculateVelocity(time) {
    if (lastAccelTime != null) {
      var deltaT = time.difference(lastAccelTime).inMicroseconds /
  ...
  void calculateVerticalVelocity(time) {
    if (lastPressTime != null && lastAltitude != null) {
      var deltaT = time.difference(lastPressTime).inMicroseconds /
          Duration.microsecondsPerSecond;
  ...
  void verifyStatus() {
    switch (currentState) {
      ...
      case rocketState.STANDBY: //Standby
        if ((rocket.altitude! - groundAltitude) > FLIGHT_ALTITUDE_TRIGGER) {
          //Launch detected :)
      ...
      case rocketState.ASCENT: // Ascent
        if (rocket.verticalVelocity! < 0) {
          // Vitesse négative -> apogée
          currentState = rocketState.DESCENT;
          rocket.apogee = rocket.altitude;
        }
        break;
      case rocketState.DESCENT: // Descent
        if (rocket.verticalVelocity! >= 0) {
          // Vitesse n'est plus négative -> atterissage
          currentState = rocketState.RECOVERY;
          dataLoop(const Duration(
              seconds: 5)); // Diminuer le taux d'envoi de données
        }
        break;
